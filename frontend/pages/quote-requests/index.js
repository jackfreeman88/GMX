...
function QuoteRequests(props) {
    ...
    useEffect(() => {
        setIsLoading(true)
        router.push('/quote-requests/', { query: { ...params } }, undefined, { shallow: true })
        const getQuoteRequests = async () => {
            const result = await Rest.axiosRequest(API.getQuoteRequests + `?limit=10&` + new URLSearchParams(params).toString(), {}, 'GET')
                        ...
                        <Col lg={12} className="mx-auto">
                            <Card className="card-dark border-gray p-8-21-30 br-10">
                                <Card.Header className="d-flex justify-content-between align-items-center border-btm-gray mb-20 p-l-0 p-r-0 p-t-0 p-b-6">
                                    <Card.Title className="fs-18 fw-600 color-dcdcdc mb-0">My Quotes</Card.Title>
                                </Card.Header>
                                ...
                                <Card.Body className="p-0">
                                    ...
                                    <div className="table-wrap CustomScrollbar CustomScrollbarY">
                                        <ResponsiveTable className="table table-bordered table-hover table-dark form-dark" id="Quotes-list">
                                            {/* <Table bordered hover variant="dark" id="quote-requests"> */}
                                            <Thead>
                                                <Tr>
                                                    <Th width="250px;">Quote ID</Th>
                                                    <Th width="250px;">Date</Th>
                                                    <Th width="250px;">Retailer Name</Th>
                                            ...
                                            <Tbody>
                                                {
                                                    isLoading ?
                                                        ...
                                                        quoteRequests.length > 0 ?
                                                            quoteRequests.map((row, index) => (
                                                                <Tr key={index + 'product'}>
                                                                    <Td>
                                                                        <Link href={"/quote-requests/" + row.quoteId}><a className="fs-14 fw-500 color-f3772c">#{row.quoteId ?? ''}</a></Link>
                                                                    </Td>
                                                                    <Td>
                                                                        <p className="fs-14 color-dcdcdc fw-500">{moment(row.createdAt).format("DD MMM 'YY")}</p>
                                                                    </Td>
                                                                    <Td>
                                                                        <Link href={"/customer/" + row.retailer.slug} ><a className="color-22a612"> {Helper.niceString(row.retailer.businessName, 20, true)} </a></Link>
                                                                    </Td>
                                                                    <Td>
                                                                        {row.totalQuantity}
                                                                    </Td>
                                                                    ...
                                                                    <Td className="text-center">
                                                                        <Link href={"/quote-requests/" + row.quoteId} ><a className="action-btn color-white text-decoration-none"> <FontAwesomeIcon icon={faEye} /></a></Link>
                                                                    </Td>
                                                                </Tr>
                                                            ))
                                                            :
