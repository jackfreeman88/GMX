...
function AddProduct() {
    ...
    // 		return true;
    // 	}else if(state.imagesExistArr.length === 0 && state.productImages.length === 0){
    // 		return true;
    // 	}else{
    // 		return false;
    // 	}
    ...
    const validator = new FormValidator([
        ...
        // },
        {
            ...
            method: "isEmpty",
            validWhen: false,
            message: "Please upload lab results.",
        }
    ]);
    ...
    useEffect(async () => {
        ...
        const strainsData = await Rest.axiosRequest(API.getStrains, {}, 'GET')
        if (strainsData.status === 200) {
            ...
            noti.notify(strainsData.data.message, "danger");
        }

        const ioData = await Rest.axiosRequest(API.getIO, {}, 'GET')
        if (ioData.status === 200) {
            ...
            noti.notify(ioData.data.message, "danger");
        }
    ...
    const addProduct = async (e) => {
        ...
        if (validation.isValid) {
            ...
            let response;
            response = await Rest.axiosRequest(API.addProduct, formData, 'POST', true)
    ...
    const documentHandler = (event) => {
        ...
        if (file) {
            ...
            if (fileType === 'application/pdf') {
                ...
                noti.notify("Please upload PDF file only.", "danger");
            }
        }
    };
