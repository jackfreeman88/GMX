...
$.fn.zabuto_calendar = function (options) {
    ...
    this.each(function () {
        ...
        function appendMonthHeader($calendarElement, $tableObj, year, month) {
            ...
            var prevHeadMonth = (month - 1);
            var prevHeadYear = year;
            if (prevHeadMonth == -1) {
                prevHeadYear = (prevHeadYear - 1);
                prevHeadMonth = 11;
            }

            var nextHeadMonth = (month + 1);
            var nextHeadYear = year;
            if (nextHeadMonth == 12) {
                nextHeadYear = (nextHeadYear + 1);
                nextHeadMonth = 0;
            }

            var $prevMonthNavIcon = $('<span><span class="fa fa-chevron-left"></span><strong>'+ moment().month(prevHeadMonth).format('MMMM') +'</strong> '+ moment().year(prevHeadYear).format('YYYY') +'</span>');
            var $nextMonthNavIcon = $('<strong>'+moment().month(nextHeadMonth).format('MMMM') +'</strong> '+ moment().year(nextHeadYear).format('YYYY') +'<span><span class="fa fa-chevron-right"></span></span>');
            ...
            var $prevMonthNav = $('<div class="calendar-month-navigation nav_prev"></div>');
            ...
            var $nextMonthNav = $('<div class="calendar-month-navigation nav_next"></div>');
            ...
            var $currMonthLabel = $('<div class="month_header"><span><strong>' + monthLabels[month] + '</strong> ' + year + '</span></div>');
            $currMonthLabel.dblclick(function () {
                var dateInitObj = $calendarElement.data('initDate');
                drawTable($calendarElement, $tableObj, dateInitObj.getFullYear(), dateInitObj.getMonth());
            });

            var $currMonthCell = $('<td colspan="7"></td>');
            $currMonthCell.append($prevMonthCell, $currMonthLabel, $nextMonthCell);
        ...
        function appendDaysOfMonth($calendarElement, $tableObj, year, month) {
            ...
            for (var wk = 0; wk < weeksInMonth; wk++) {
                ...
                for (var dow = 0; dow < 7; dow++) {                    
                    if (dow < firstDow) {
                        var prevcurrentday = calcLastDayInMonth(prevHeadYear, prevHeadMonth) + (dow - (firstDow - 1));
                        $dowRow.append('<td class="disable">'+prevcurrentday+'</td>');
                    }else if(currDayOfMonth > lastDayinMonth){
                        $dowRow.append('<td class="disable">'+nextfirstday+'</td>');
                        nextfirstday++;
                    }else {
                        var todaydate = moment().zone('+11').format('YYYY-MM-DD');
                        var currentdate = moment(dateAsString(year, month, currDayOfMonth)).zone('+11').format('YYYY-MM-DD')
                        if (currentdate >= todaydate){
                            ...
                            $dowElement.attr('data-date', dateAsString(year, month, currDayOfMonth));
                            ...
                            var $dowElement = $('<td class="disable">' + currDayOfMonth + '</td>');
                            $dowElement.append($dayElement);
                        }
        ...
        function ajaxEvents($calendarElement, year, month) {
            ...
            var data = {year: year, month: moment().month(month).format('MM')};
            if(ajaxSettings.data){
                $.each(ajaxSettings.data, function(key, value) {
                    var k = key.toString();
                    data[k] = value;
                });
            }
            ...
            $.ajax({
                type: (ajaxSettings.type) ? ajaxSettings.type : 'GET',
            ...
            if (events !== false) {
                $(events).each(function (index, value) {
                    ...
                    if (typeof(value.classname) === 'undefined') {
                        ...
                        if(value.classname == "calendar-available"){
                            if (typeof($calendarElement.data('actionFunction')) === 'function') {
                                $dowElement.addClass('dow-clickable');
                                $dowElement.click(function () {
                                    $calendarElement.data('selectedDate', $(this).data('date'));
