...
<div class="user-wrap">
    ...
    @else 
        @php $id = 0 @endphp
    @endif
    {!! Form::open(['url'=>url($actionURL.'/action',$view).'/'.$id,'files'=>true,'id'=>'med_recform']) !!}
    ...
    <div class="user-wrap">
        <div class="card bs-4 mb-3 card-dark">
            <div class="card-body">
                ...
                {!! Form::open(['url'=>url($actionURL),'method'=>'get','id'=>'med_recFilter']) !!}
                    <div class="row justify-content-lg-end">
                        ...
                        <div class="form-group">
                            ...
                            @if( isset($data['filter']) && !empty($data['filter']) )
                                <a href="{{ url('med_rec') }}" data-toggle="tooltip" title="Clear" class="btn btn-secondary1 btn-wh-50-46 br-6"><i class="fas fa-times-circle"></i></a>
                            @endif
                        </div>
                    </div>
                    </div>
                    ...
                    <div class="tbl-body">
                        <div class="table-responsive1">
                            <table class="table  fs-14 reflow-tbl">
                                ...
                                <tbody>
                                    @if (isset($med_recs) && count($med_recs) > 0)
                                        @foreach ($med_recs as $med_rec)
                                            <tr>
                                                <td>{{ $med_rec->title ?? '' }}</td>
                                                <td>{{ ($med_rec->isActive=='1')?'Active':'Inactive' }}</td>
                                                <td>
                                                    <div class="d-flex text-center">
                                                        <a title="Edit" href="{{url($actionURL.'/edit/'.$med_rec->id)}}"><i class="fas fa-edit text-success fs-18"></i> </a>
                                                        <span class="mx-1">|</span>
                                                        <a title="Delete" href="javascript:void(0)" onclick="gmx_modal.confirmModal('{{ url($actionURL.'/action/delete/'.$med_rec->id) }}');"><i class="fas fa-trash-alt fs-18 text-danger"></i>
                                                        </a>
                                                    </div>
                                                </td>
                                            </tr>
