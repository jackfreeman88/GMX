...
<div class="user-wrap">
    ...
    @else 
        @php $id = 0 @endphp
    @endif
    {!! Form::open(['url'=>url($actionURL.'/action',$view).'/'.$id,'files'=>true,'id'=>'emailfooterform']) !!}
                ...
                <div class="tbl-rounded">
                    <div class="tbl-body">
                        <div class="table-responsive1">
                            <table id="email_footers_table" class="table  fs-14 reflow-tbl">
                                <thead>
                                ...
                                <tbody>
                                    @if (isset($emailFooters) && count($emailFooters) > 0)
                                        @foreach ($emailFooters as $emailFooter)
                                            <tr>
                                                <td>{{ $emailFooter->title ?? '' }}</td>
                                                <td>{{ ($emailFooter->status=='1')?'Active':'Inactive' }}</td>
                                                <td>
                                                    <div class="d-flex text-center">
                                                        @if ($emailFooter->status == '1')
                                                            <a title="Make Inactive" href="{{url($actionURL.'/action/makeInactive/'.$emailFooter->id)}}"><i class="fas fa-check fs-18 text-success"></i> </a>
                                                        @else
                                                            <a title="Make Active" href="{{url($actionURL.'/action/makeActive/'.$emailFooter->id)}}"><i class="far fa-times-circle fs-18 color-0799c4"></i> </a>
                                                        @endif
                                                        <span class="mx-1">|</span>
                                                        <a title="Edit" href="{{url($actionURL.'/edit/'.$emailFooter->id)}}"><i class="fas fa-edit fs-18 text-success"></i> </a>
                                                        <span class="mx-1">|</span>
                                                        <a title="Delete" href="javascript:void(0)" onclick="mechanech_modal.confirmModal('{{ url($actionURL.'/action/delete/'.$emailFooter->id) }}');"><i class="far fa-trash-alt fs-18 text-danger"></i>
                                                        </a>
                                                    </div>
                                                </td>
                                            </tr>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        gmx_app.getckeditor($('#description'),ck_config);
    });
</script>
@stop