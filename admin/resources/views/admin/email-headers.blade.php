...
<div class="user-wrap">
    ...
    @else 
        @php $id = 0 @endphp
    @endif
    {!! Form::open(['url'=>url($actionURL.'/action',$view).'/'.$id,'files'=>true,'id'=>'emailheaderform']) !!}
                ...
                <div class="tbl-rounded">
                    <div class="tbl-body">
                        <div class="table-responsive1">
                            <table id="email_headers_table" class="table  fs-14 reflow-tbl">
                                <thead>
                                ...
                                <tbody>
                                    @if (isset($emailHeaders) && count($emailHeaders) > 0)
                                        @foreach ($emailHeaders as $emailHeader)
                                            <tr>
                                                <td>{{ $emailHeader->title ?? '' }}</td>
                                                <td>{{ ($emailHeader->status=='1')?'Active':'Inactive' }}</td>
                                                <td>
                                                    <div class="d-flex text-center">
                                                        @if ($emailHeader->status == '1')
                                                            <a title="Make Inactive" href="{{url($actionURL.'/action/makeInactive/'.$emailHeader->id)}}"><i class="fas fa-check fs-18 text-success"></i> </a>
                                                        @else
                                                            <a title="Make Active" href="{{url($actionURL.'/action/makeActive/'.$emailHeader->id)}}"><i class="fas fa-times-circle fs-18 color-0799c4"></i> </a>
                                                        @endif
                                                        <span class="mx-1">|</span>
                                                        <a title="Edit" href="{{url($actionURL.'/edit/'.$emailHeader->id)}}"><i class="fas fa-edit text-success fs-18"></i> </a>
                                                        <span class="mx-1">|</span>
                                                        <a title="Delete" href="javascript:void(0)" onclick="mechanech_modal.confirmModal('{{ url($actionURL.'/action/delete/'.$emailHeader->id) }}');"><i class="fas fa-trash-alt fs-18 text-danger"></i>
                                                        </a>
                                                    </div>
                                                </td>
                                            </tr>
